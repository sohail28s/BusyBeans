...
export const CustomersReportTable = ({ data, isLoading, onDownloadCSV }) => {
    ...
    const sortedData = useMemo(() => {
        ...
        if (sortConfig.key && sortConfig.direction !== 'default') {
            sortableItems.sort((a, b) => {
                ...
                // All sortable columns in this specific table are numerical
                ...
                <table className="w-full text-left whitespace-nowrap border-collapse min-w-[1200px]">
                    <thead className="bg-[#f9fafb]">
                        <tr className="border-b border-[#e2e8f0]">
                            ...
                            {/* Rigid Widths Applied to all Headers */}
                            <th className="px-8 py-5 font-bold text-[#374151] text-[14px] min-w-[200px]">Customer Name</th>
                            <th className="px-8 py-5 font-bold text-[#374151] text-[14px] min-w-[250px]">Company Name</th>
                            
                            <SortableHeader label="No of Orders" sortKey="numberOfOrders" currentSort={sortConfig} onSort={handleSort} width="min-w-[150px]" />
                            <th className="px-8 py-5 font-bold text-[#374151] text-[14px] min-w-[180px]">Last Order Date</th>
                            <SortableHeader label="Outstanding Balance" sortKey="outstandingBalance" currentSort={sortConfig} onSort={handleSort} width="min-w-[200px]" />
                            <SortableHeader label="Avg. Spent" sortKey="avgSpent" currentSort={sortConfig} onSort={handleSort} width="min-w-[150px]" />
                            <SortableHeader label="Total Spent" sortKey="totatSpent" currentSort={sortConfig} onSort={handleSort} width="min-w-[150px]" />
                        </tr>
                    </thead>
                    <tbody>
                        {isLoading ? (
                            <tr><td colSpan="8" className="text-center py-12 text-[#4b5563] text-[15px] italic">Loading data...</td></tr>
                            ...
                            paginatedData.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="px-8 py-5 text-gray-900 truncate" title={item.name}>{item.name || '-'}</td>
                                    <td className="px-8 py-5 text-gray-900 truncate" title={item.companyName}>{item.companyName || '-'}</td>
                                    <td className="px-8 py-5">{item.numberOfOrders || 0}</td>
                                    <td className="px-8 py-5">{item.lastOrderDate || '-'}</td>
                                    <td className="px-8 py-5">{formatMoney(item.outstandingBalance)}</td>
                                    <td className="px-8 py-5">{formatMoney(item.avgSpent)}</td>
                                    <td className="px-8 py-5">{formatMoney(item.totatSpent)}</td>
                                </tr>
                            ))
                        ) : (
                            <tr><td colSpan="8" className="text-center py-12 text-[#4b5563] text-[15px] italic">No records found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
