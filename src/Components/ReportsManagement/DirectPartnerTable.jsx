...
export const DirectPartnerTable = ({ data, isLoading, onDownloadCSV }) => {
    ...
    const sortedData = useMemo(() => {
        ...
        if (sortConfig.key && sortConfig.direction !== 'default') {
            sortableItems.sort((a, b) => {
                ...
                if (sortConfig.key === 'territoryName') {
                    aVal = (aVal || '').toString().toLowerCase();
                    bVal = (bVal || '').toString().toLowerCase();
                } else {
                    // Numerical sorting for ID and Counts
                ...
                <table className="w-full text-left whitespace-nowrap border-collapse">
                    <thead className="bg-[#f9fafb]">
                        <tr className="border-b border-[#e2e8f0]">
                            ...
                            <th className="px-8 py-5 font-bold text-[#374151] text-[14px] min-w-[250px]">Partner Name</th>
                            <SortableHeader label="Territory Name" sortKey="territoryName" currentSort={sortConfig} onSort={handleSort} width="min-w-[200px]" />
                            <SortableHeader label="Client Orders" sortKey="clientOrdersCount" currentSort={sortConfig} onSort={handleSort} width="min-w-[150px]" />
                            <th className="px-8 py-5 font-bold text-[#374151] text-[14px] min-w-[150px]">Self Order</th>
                            {/* Sorted by clientOrdersCount as instructed */}
                            <SortableHeader label="Total Orders" sortKey="clientOrdersCount" currentSort={sortConfig} onSort={handleSort} width="min-w-[150px]" />
                        </tr>
                    </thead>
                    <tbody>
                        {isLoading ? (
                            ...
                            paginatedData.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="px-8 py-5">{item.territoryName || '-'}</td>
                                    <td className="px-8 py-5">{item.clientOrdersCount || 0}</td>
                                    <td className="px-8 py-5">{item.selfOrdersCount || 0}</td>
                                    {/* Displaying clientOrdersCount here as requested */}
                                    <td className="px-8 py-5">{item.clientOrdersCount || 0}</td>
                                </tr>
                            ))
                        ) : (
