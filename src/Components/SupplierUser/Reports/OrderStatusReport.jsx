...
const OrderStatusReport = () => {
    ...
    // Set Page Title
    useEffect(() => {
        setStoreTitle('Order Status Report');
    ...
    useEffect(() => {
        const fetchReportData = async () => {
            ...
            try {
                // IMPORTANT: Replace this URL if your specific order status report endpoint is different!
                ...
                console.error("Failed to fetch order status report:", error);
            } finally {
                setIsLoading(false);
    ...
    const filteredReports = reports.filter(item => {
        ...
        return (
            ...
            (item.orderDate || item.createdAt || '').toLowerCase().includes(q) ||
    ...
    const handleDownloadCSV = () => {
        ...
        const csvFormatted = displayData.map(row => ({
            ...
            "Order Date": row.orderDate || row.createdAt || "—",
            "Order Status": row.orderCurrentStatus || "—",
            "Assigned At": row.assignedAt || "—",
            "Note": row.note || "—"
        }));
        exportToCSV(csvFormatted, `Order_Status_Report_${startDate}_to_${endDate}.csv`);
    };
                ...
                <div className="bg-white w-full border-[0.66px] border-[#e2e8f0] rounded-[12px] shadow-sm p-6 flex flex-col relative">
                    ...
                    <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
                        <div className="relative flex items-center w-full sm:w-[350px] h-[45px] bg-[#f9fafb] border-[0.66px] border-[#e5e7eb] rounded-[8px] overflow-hidden">
                            ...
                            <input 
                                type="search" 
                                placeholder="Search by SL, Name, Status..." 
                                value={searchQuery} 
                    ...
                    <div className="w-full overflow-x-auto">
                        <table className="w-full text-left border-none min-w-[900px]">
                            <thead className="bg-[#f9fafb]">
                                <tr>
                                    <SortableHeader label="SL" sortKey="sl" currentSort={sortConfig} onSort={handleSort} width="w-[80px]" />
                                    <th className="px-6 py-5 font-bold text-[#374151] text-[14px] w-[300px] border-none">Customer Name</th>
                                    <th className="px-6 py-5 font-bold text-[#374151] text-[14px] w-[250px] border-none">Order Date</th>
                                    <th className="px-6 py-5 font-bold text-[#374151] text-[14px] w-[270px] border-none last:rounded-r-lg">Order Status</th>
                            ...
                            <tbody className="border-none">
                                {isLoading ? (
                                    <tr><td colSpan="4" className="text-center py-12 text-gray-500 italic border-none">Loading report...</td></tr>
                                    ...
                                    displayData.map((row) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-5 border-none align-top whitespace-normal break-words">
                                                {row.customerName || "—"}
                                            </td>
                                            
                                            {/* Order Date: Rendered raw as requested */}
                                            <td className="px-6 py-5 border-none align-top whitespace-normal break-words">
                                                {row.orderDate || row.createdAt || "—"}
                                            </td>
                                            
                                            {/* Order Status: Simple text */}
                                            <td className="px-6 py-5 border-none align-top whitespace-normal break-words capitalize text-gray-600">
                                    ...
                                    <tr><td colSpan="4" className="text-center py-12 text-gray-500 italic border-none">No records found.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
