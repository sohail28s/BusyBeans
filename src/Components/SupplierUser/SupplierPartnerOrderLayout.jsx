...
export const SupplierPartnerOrderLayout = ({ apiEndpoint, detailsBaseRoute }) => {
    ...
    const handleDownloadCSV = () => {
        ...
        const csvFormatted = displayData.map(order => ({
            ...
            "No of Items": calculateTotalItems(order.partnerOrderItems),
            "Status": order.orderCurrentStatus
        }));
        exportToCSV(csvFormatted, `Supplier_Orders_Page_${pagination.page}.csv`);
    };
            ...
            <div className="bg-white w-full border-[0.66px] border-[#e2e8f0] rounded-[12px] shadow-sm p-6 flex flex-col relative">
                ...
                <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
                    <div className="relative flex items-center w-[350px] h-[45px] bg-[#f9fafb] border-[0.66px] border-[#e5e7eb] rounded-[8px] overflow-hidden">
                        ...
                        <input type="search" placeholder="Search by ID, Partner Name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full h-full bg-transparent text-[14px] text-gray-700 focus:outline-none pr-3" />
                    </div>
                    ...
                    <table className="w-full text-left min-w-max border-none">
                        <thead className="bg-[#f9fafb]">
                            <tr>
                                ...
                                <th className="px-8 py-5 font-bold text-[#374151] text-[14px] w-[250px] border-none">Partner Name</th>
                        ...
                        <tbody className="border-none">
                            {isLoading ? (
                                ...
                                displayData.map((order) => {
                                    ...
                                    const totalItemsCount = calculateTotalItems(order.partnerOrderItems);
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="px-8 py-5 border-none font-medium">{order.salesRepName || "—"}</td>
