...
export const PartnerCommissionTable = ({ data, isLoading, onDownloadCSV }) => {
                ...
                <table className="w-full text-left whitespace-nowrap border-collapse">
                    <thead className="bg-[#f9fafb]">
                        <tr className="border-b border-[#e2e8f0]">
                            ...
                            <SortableHeader label="Local Partner" sortKey="srName" currentSort={sortConfig} onSort={handleSort} width="min-w-[250px]" />
                            <SortableHeader label="Orders Placed" sortKey="ordersPlaced" currentSort={sortConfig} onSort={handleSort} width="min-w-[150px]" />
                            <SortableHeader label="Total Sales" sortKey="totalSales" currentSort={sortConfig} onSort={handleSort} width="min-w-[150px]" />
                            <SortableHeader label="Whole Sale Price Cost" sortKey="wholesalePriceCost" currentSort={sortConfig} onSort={handleSort} width="min-w-[200px]" />
                            <SortableHeader label="Total Partner Profits" sortKey="totalCommission" currentSort={sortConfig} onSort={handleSort} width="min-w-[200px]" />
                        </tr>
                    </thead>
                    <tbody>
                        {isLoading ? (
                            <tr><td colSpan="6" className="text-center py-12 text-[#4b5563] text-[15px] italic">Loading data...</td></tr>
                            ...
                            paginatedData.map((item, index) => (
                                ...
                                >
                                    {/* Calculated SL Number based on pagination */}
                                    ...
                                    <td className="px-8 py-5">{formatMoney(item.ordersPlaced)}</td>
                                    <td className="px-8 py-5">{formatMoney(item.totalSales)}</td>
                                    <td className="px-8 py-5">{formatMoney(item.wholesalePriceCost)}</td>
                                    <td className="px-8 py-5">{formatMoney(item.totalCommission)}</td>
                                </tr>
                            ))
                        ) : (
                            <tr><td colSpan="6" className="text-center py-12 text-[#4b5563] text-[15px] italic">No records found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
