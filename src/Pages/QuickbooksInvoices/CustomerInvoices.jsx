...
const CustomerInvoices = () => {
    // Access your Zustand store
    const setStoreTitle = useStore(state => state.setTitle);
    const setStoreShowProfile = useStore(state => state.setShowProfile);
      const setIsGlobalLoading = useStore(state => state.setIsGlobalLoading); // Global loader added


    // Set Navbar Title and Hide Profile on mount
    useEffect(() => {
        setStoreTitle('Quickbook Customer Invoices');
    ...
    const syncLabel = currentFilters.syncStatus === 'not-synced' ? 'Sync Invoices' : 'Sync Payment';
     ...
     useEffect(() => { 
        const fetchInvoices = async () => { 
            setIsLoading(true); 
            setIsGlobalLoading(true); // Trigger global loader
            try { 
                const baseUrl = `https://testingbb.trimworldwide.com/api/v1/admin/quickbooks-customer-order-management/${currentFilters.syncStatus}`; 
    ...
    return (
        <div className="w-full min-h-[calc(100vh-100px)] bg-whitep-6 md:p-8 font-sans">
            {/* Removed the local H1 header since it's now in the TopNavbar! */}
