...
import { DirectPartnerTable } from '../../ComponentsTemp/ReportsManagement/DirectPartnerTable'; // Adjust path if needed
...
export const DirectPartnerPage = () => {
    ...
    // --- Header Setup ---
    useEffect(() => {
        setTitle('Direct Partner Report');
        setActions(null);
    ...
    const fetchReportData = useCallback(async () => {
        ...
        try {
            const apiUrl = `https://testingbb.trimworldwide.com/api/v1/admin/admin-reports/direct-partner-summary?startDate=${startDate}&endDate=${endDate}`;
    ...
    const handleDownloadCSV = (filteredData) => {
        ...
        const csvFormatted = filteredData.map((item, index) => {
            return {
                "SL": index + 1,
                "Partner Name": item.srName || "-",
                "Territory Name": item.territoryName || "-",
                "Client Orders": item.clientOrdersCount || 0,
                "Self Order": item.selfOrdersCount || 0,
                "Total Orders": item.clientOrdersCount || 0 // Mapped to clientOrdersCount as requested
            };
        });

        exportToCSV(csvFormatted, `Direct_Partner_Report_${startDate}_to_${endDate}.csv`);
    };
