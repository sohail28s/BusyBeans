...
export const PartnerCreditLimitPage = () => {
    ...
    // --- Header Setup ---
    useEffect(() => {
        setTitle('Partner Credit Limit Report');
        setActions(null);
    ...
    const fetchReportData = useCallback(async () => {
        ...
        try {
            const apiUrl = `https://testingbb.trimworldwide.com/api/v1/admin/admin-reports/partner-creadit-limit?startDate=${startDate}&endDate=${endDate}`;
    ...
    const handleDownloadCSV = (filteredData) => {
        ...
        const csvFormatted = filteredData.map((item, index) => {
            // Safe calculation for CSV
            const u = parseFloat(item.creditUsed || 0);
            const l = parseFloat(item.creditLimit || 0);
            const utilization = l === 0 || isNaN(l) ? 0 : (u / l) * 100;

            return {
                "SL": index + 1,
                "Local Partner": item.srName || "-",
                "Credit Limit": item.creditLimit ? `$${item.creditLimit.toFixed(2)}` : "$0.00",
                "Credit Used": item.creditUsed ? `$${item.creditUsed.toFixed(2)}` : "$0.00",
                "Credit Utilization (%)": `${utilization.toFixed(2)}%`
            };
        });

        exportToCSV(csvFormatted, `Partner_Credit_Limit_Report_${startDate}_to_${endDate}.csv`);
    };
...
export default PartnerCreditLimitPage;