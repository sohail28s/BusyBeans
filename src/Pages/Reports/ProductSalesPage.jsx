...
import { ProductSalesTable } from '../../ComponentsTemp/ReportsManagement/ProductSalesTable'; 
...
export const ProductSalesPage = () => {
    ...
    // --- Header Setup ---
    useEffect(() => {
        setTitle('Products Sales Report');
        setActions(null);
        setShowProfile(false); // Hide profile as requested

        return () => {
            setTitle('');
            setShowProfile(true); // Restore profile on unmount
        };
    ...
    const fetchReportData = useCallback(async () => {
        ...
        try {
            const apiUrl = `https://testingbb.trimworldwide.com/api/v1/admin/admin-reports/product-sales?startDate=${startDate}&endDate=${endDate}`;
    ...
    const handleDownloadCSV = (filteredData) => {
        ...
        const csvFormatted = filteredData.map((item, index) => {
            return {
                ...
                "Revenue From Customer Orders": item.revenueFromCustomers ? `$${parseFloat(item.revenueFromCustomers).toFixed(2)}` : "$0.00",
                "Units Sold": item.totalUnits,
                "Revenue From Local Partner Self Orders": item.revenueFromLocalPartners ? `$${parseFloat(item.revenueFromLocalPartners).toFixed(2)}` : "$0.00",
                "Admin Receivable From Local Partner": item.wholesalePriceTotal ? `$${parseFloat(item.wholesalePriceTotal).toFixed(2)}` : "$0.00",
                "Units Sold To Customers": item.unitsSoldToCustomer || 0,
                "Units Sold To Partners": item.unitsSoldToPartners || 0
            };
        });

        exportToCSV(csvFormatted, `Product_Sales_Report_${startDate}_to_${endDate}.csv`);
    };
